...
BEGIN
 ...
 BEGIN
 DECLARE @session_code NVARCHAR(100);
 DECLARE @game_format NVARCHAR(50);
 DECLARE @start_time DATETIME2;
 DECLARE @end_time DATETIME2;

 SET @session_code = @Xml.value('(/session/@sessioncode)[1]', 'NVARCHAR(100)');
 SET @game_format = @Xml.value('(/session/general/gametype)[1]', 'NVARCHAR(50)');
 SET @start_time = @Xml.value('(/session/general/startdate)[1]', 'DATETIME2');
 SET @end_time = @Xml.value('(/session/general/enddate)[1]', 'DATETIME2');
