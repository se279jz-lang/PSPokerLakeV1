...
<# 
•	Added param() block with [string]$HistoryDir and [string]$ConfigFile.
•	If HistoryDir is relative it's resolved relative to the script folder; otherwise absolute paths are supported.
•	Same behavior for ConfigFile.
•	Added diagnostic output showing which history directory and config file were used and a warning if the history directory doesn't exist.
•	Existing XML structure and historytable creation logic preserved.
    Usage examples:
•	Default: .\01_create-config.ps1
•	Custom history dir: .\01_create-config.ps1 -HistoryDir 'C:\path\to\history'
•	Custom history dir: .\01_create-config.ps1 -HistoryDir 'C:\Users\webm\Desktop\Data'
•	Custom history dir AND config file: .\01_create-config.ps1 -HistoryDir 'C:\Users\webm\Desktop\Data' -ConfigFile 'C:\Users\webm\Desktop\PSWorks\config.xml'
...
•	Custom config file: .\01_create-config.ps1 -ConfigFile 'C:\Users\webm\Desktop\PSWorks\config.xml'
#>