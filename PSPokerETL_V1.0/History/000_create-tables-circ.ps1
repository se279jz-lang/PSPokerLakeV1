...
$instanceName = $config.config.InstanceName.Trim()
$databaseName = $config.config.DatabaseName.Trim()
$tables = $config.config.historytable
$server = "(localdb)\$instanceName"

Write-Host "⛏️ Creating tables in '$databaseName' on $server"

# 🧬 Table schema template
$schema = @"
IF OBJECT_ID(N'dbo.{0}', 'U') IS NULL BEGIN
    CREATE TABLE dbo.{0} (
...
-- 🛡️ Add unique constraints if missing
IF OBJECT_ID(N'dbo.{0}', 'U') IS NOT NULL BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM sys.indexes
        WHERE name = N'UQ_{0}_Sha256Hash'
    ...
    IF NOT EXISTS (
        SELECT 1 FROM sys.indexes
        WHERE name = N'UQ_{0}_SessionCode'
        AND object_id = OBJECT_ID(N'dbo.{0}')
    )
...
Write-Host "✅ Tables created and constraints minted."
